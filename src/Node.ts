...
export function start() {
  ...
  liveQuery(async () => {
    ...
    return await db.notes
      .filter(
        (note) =>
          note.checkAt?.toString() === new Date(0).toString() &&
          note.id !== actnote?.id
      )
      .count();
...
async function checkSync() {
  ...
  // 拉取服务器文件
  const upbooks = await db.books
    .orderBy('checkAt')
    .filter((book) => !book.checkAt || book.checkAt < lastAt)
    .toArray();
    ...
    try {
      ...
      if (!book.enabled) {
        ...
        if (stat.cid.toString() !== book.hash) {
          ...
          // 删除本地
          const removes = await db.notes
            .where('bookId')
            .equals(book.id!)
            .filter((note) =>
              files.every(
                (file) =>
                  file.path !== '/' + book.name + '/' + note.name &&
                  note.syncAt &&
                  note.syncAt > note.updateAt!
              )
            );
